...
export const EventsView: React.FC<EventsViewProps> = ({ workItems, events, onViewEvent, onAddNewEvent, scope, onScopeChange }) => {
    const { can } = useBoard();
    const { t } = useLocale();
                ...
                <div className="flex items-center gap-4">
                     ...
                     {isScrumMaster && (
                        <div className="flex items-center gap-2 p-1 bg-gray-100 rounded-lg">
                             <button onClick={() => onScopeChange('my')} className={`px-3 py-1 text-sm rounded-md ${scope === 'my' ? 'bg-white shadow-sm' : ''}`}>{t('my_events')}</button>
                             <button onClick={() => onScopeChange('all')} className={`px-3 py-1 text-sm rounded-md ${scope === 'all' ? 'bg-white shadow-sm' : ''}`}>{t('all_events')}</button>
                        </div>
                     )}
                </div>
                <div className="flex items-center gap-4">
