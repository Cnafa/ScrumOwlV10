...
interface TeamsTabProps {
    teams: Team[];
    onSaveTeam: (team: Partial<Team>, memberIds: string[]) => void;
    onDeleteTeam: (teamId: string) => void;
    allMembers: BoardMember[];
}

export const TeamsTab: React.FC<TeamsTabProps> = ({ teams, onSaveTeam, onDeleteTeam, allMembers }) => {
    const { t } = useLocale();
    ...
    const handleSaveTeam = (teamToSave: Partial<Team>, memberIds: string[]) => {
        onSaveTeam(teamToSave, memberIds);
        setIsEditorOpen(false);
        setEditingTeam(null);
    };

    const handleSaveMembers = (teamId: string, memberIds: string[]) => {
        onSaveTeam({ id: teamId }, memberIds);
